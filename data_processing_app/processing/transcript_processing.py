...
def main(sentiments_from_text):

    report_file = open("report/MeetingNotesAndSentimentsFromText.txt", "w")

    convertfiletocsv()
    #findSentiments()
    if sentiments_from_text:
        sentimentsReport = sentimentsAnalysis()
        report_file.write('Sentiments Report \n =====\n '+sentimentsReport+'\n\n\n')
    
    agendaFinal,meetingNotesFinal,nextStepsFinal = meetingNotes()
    
    report_file.write(agendaFinal+'\n\n')
    report_file.write(meetingNotesFinal + '\n\n')
    report_file.write(nextStepsFinal)
...
def meetingNotes():
    ...
    try:
        ...
        return agenda,meetingNotes,nextSteps
